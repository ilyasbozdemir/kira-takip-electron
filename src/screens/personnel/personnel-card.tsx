...
export const PersonnelCard: React.FC<PersonnelCardProps> = ({
  ...
  return (
    <Card
      className={`transition-all hover:border-indigo-500/50 flex flex-col justify-between ${
    ...
    >
      <div>
        <CardHeader className="pb-2.5 pt-4 px-4 flex flex-row items-start justify-between space-y-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className={`text-sm font-bold truncate max-w-45 ${isDark ? "text-slate-100" : "text-slate-900"}`}>
                {p.name}
              </h4>
              <p className="text-[11px] text-indigo-400 font-medium truncate max-w-45 flex items-center gap-1">
                <Briefcase className="h-3 w-3 shrink-0" /> {p.title || "Görevli"}
              </p>
            </div>
          </div>
          {assignedVenues.length > 0 && (
            <Badge variant="outline" className="text-[10px] bg-emerald-500/10 border-emerald-500/30 text-emerald-400 font-bold shrink-0">
        ...
        <CardContent className="px-4 pb-4 space-y-2.5 text-xs">
              ...
              <div className="flex items-start gap-2 pt-0.5">
                ...
                <div className="flex flex-wrap gap-1">
                  {assignedVenues.map((v) => (
                    <Badge key={v.id} variant="outline" className="text-[9px] border-slate-700 px-1 py-0 font-normal text-slate-300">
                      🏛️ {v.name}
                    </Badge>
                  ))}
                </div>
          ...
          {p.notes && (
            <p className={`text-[11px] p-2 rounded-lg border leading-snug line-clamp-2 font-medium ${
              isDark ? "bg-slate-950/60 border-slate-800 text-slate-300" : "bg-slate-100 border-slate-300 text-slate-800"
            }`}>
              💬 {p.notes}
            </p>
          )}
      ...
      <div className={`p-4 pt-2 border-t flex items-center justify-between gap-1.5 ${
        ...
        <div>
          {p.phone && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-2.5 py-1 rounded-md bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-500/10 dark:border dark:border-emerald-500/30 dark:text-emerald-400 text-[10px] font-bold transition-colors shadow-2xs flex items-center gap-1"
            >
