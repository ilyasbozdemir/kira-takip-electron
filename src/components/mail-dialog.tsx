...
function generateSingleReservationICS(resData?: any) {
  const data = resData && resData.customer
    ? resData
    : DEFAULT_SAMPLE_RESERVATION;
  const dateStr = (data.date || new Date().toISOString().split("T")[0]).replace(
    /-/g,
    "",
  );
...
export function MailDialog({
  ...
  const handleDownloadICS = () => {
    try {
      ...
      link.setAttribute(
        "download",
        `etkinlik-${reservationData?.id || "tahsisi"}.ics`,
      );
  ...
  const handleSendEmail = async () => {
    ...
    if (!smtpConfig.user.trim() || !smtpConfig.pass.trim()) {
      toast.error(
        "E-posta göndermek için lütfen önce Ayarlar > E-posta sekmesinden SMTP kullanıcı adı ve şifrenizi tanımlayın.",
      );
      return;
    }
    ...
    try {
      ...
      const attachments = attachIcs
        ? [
          {
            filename: `etkinlik-takvim-daveti-${
              reservationData?.id || "tahsisi"
            }.ics`,
            content: icsContent,
      ...
      if (window.electronAPI?.sendEmail) {
        ...
        toast.info(
          "E-posta gönderme özelliği masaüstü Electron ortamında aktiftir.",
        );
      }
    } catch (err: any) {
      ...
      >
        <DialogHeader>
          <div className="flex items-center justify-between">
            ...
            >
              ...
              E-posta & Takvim İletimi
            </DialogTitle>
            {isSmtpConfigured
              ? (
                <Badge
                  variant="outline"
                  className="bg-emerald-500/10 border-emerald-500/30 text-emerald-400 text-[10px] font-bold"
                >
              ...
              : (
                <Badge
                  variant="outline"
                  className="bg-amber-500/10 border-amber-500/30 text-amber-400 text-[10px] font-bold"
                >
          ...
          >
            Rezervasyon dökümünü e-posta olarak gönderin veya .ics takvim
            dosyasını indirin.
          </DialogDescription>
        </DialogHeader>
        ...
        {!isSmtpConfigured && activeTab === "send" && (
          <div className="p-3 mb-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs flex items-center justify-between">
            <span>
              ⚠️ E-posta göndermek için henüz SMTP sunucu ayarları
              tanımlanmamış.
            </span>
          </div>
        )}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <span className="font-bold text-emerald-400 block text-[11px]">
                        Otomatik Eklenen Takvim Dosyası:{" "}
                        <code className="text-slate-200">
                          etkinlik-takvim-daveti.ics
                        </code>
                      </span>
                      <span className="text-[10px] text-slate-300 font-medium">
                        👤{" "}
                        {reservationData?.customer ||
                          DEFAULT_SAMPLE_RESERVATION.customer} • 📅{" "}
                        {reservationData?.date ||
                          DEFAULT_SAMPLE_RESERVATION.date}{" "}
                        ({reservationData?.start ||
                          DEFAULT_SAMPLE_RESERVATION.start}-{reservationData
                          ?.end || DEFAULT_SAMPLE_RESERVATION.end})
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex items-start space-x-2.5 p-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10">
                ...
                <div className="grid gap-0.5 leading-none">
                  ...
                  >
                    ...
                    .ics Takvim Daveti Dosyasını E-postaya Ekle (Attachment)
                  </Label>
                  <p className="text-[11px] text-slate-400 leading-snug">
                    İşaretlendiğinde alıcının Gmail veya Outlook posta kutusunda
                    tıklanabilir <b>"Takvime Ekle"</b> düğmesi otomatik çıkar.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3.5 py-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs font-bold text-emerald-400">
                  ...
                  Görsel Takvim Daveti & Kart Önizleme
                </div>
                <div className="flex items-center gap-1.5 bg-slate-950/60 p-0.5 rounded-lg border border-slate-800">
                  <button
                    type="button"
                        ...
                        <div>
                          ...
                          >
                            {reservationData?.eventType ||
                              "Salon Tahsis Etkinliği"}:{" "}
                            {reservationData?.customer ||
                              "Müşteri Tahsis Kaydı"}
                          </h3>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="text-[11px] font-mono text-emerald-500 font-medium mt-0.5">
                          ⏰ {reservationData?.start || "09:00"} -{" "}
                          {reservationData?.end || "17:00"}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-slate-800/80 flex-wrap">
                      <a
                        href={(() => {
                          ...
                          const startStr =
                            (data.start || "18:00").replace(":", "") + "00";
                          const endStr =
                            (data.end || "23:30").replace(":", "") + "00";
                          const dtStart = `${dateStr}T${startStr}`;
                          const dtEnd = `${dateStr}T${endStr}`;
                          const title = encodeURIComponent(
                            `${data.eventType || "Salon Kiralaması"}: ${
                              data.customer || "Müşteri"
                            }`,
                          );
                          const details = encodeURIComponent(
                            `Müşteri: ${data.customer || "-"} | Tel: ${
                              data.phone || "-"
                            }`,
                          );
                          const location = encodeURIComponent(
                            `${data.venueName || "Tesis"} - ${
                              data.hallName || "Salon"
                            }`,
                          );
