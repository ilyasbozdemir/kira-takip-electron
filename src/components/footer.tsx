...
import { Bug, ExternalLink, Info, Star, Wifi, Database, Building2 } from "lucide-react";
...
export function Footer({ currentFilePath, institutionName, theme = "dark" }: FooterProps): React.JSX.Element {
    ...
    >
      ...
      <div className="flex items-center space-x-2">
        ...
        {institutionName && (
          <>
            ...
            <span className="truncate max-w-[180px] font-medium flex items-center gap-1" title={institutionName}>
        ...
        <span className="text-[11px] opacity-75">
          Son Senkronizasyon: <span className="font-mono font-semibold">{new Date().toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" })}</span>
        </span>
      </div>
      ...
      <div className="flex items-center space-x-2 relative" ref={aboutRef}>
        ...
        <button
          ...
          className="flex items-center space-x-1 transition-colors cursor-pointer px-2 py-0.5 rounded-md hover:bg-emerald-500/10 text-emerald-500 font-medium text-[11px]"
          title="Yerel Ağ Senkronizasyonu & İletişim Durumu"
        >
        ...
        {showAbout && (
          ...
          >
            <div className="p-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 flex justify-between items-start">
              <div>
                <h4 className="font-bold text-sm text-indigo-400">İşletmeTakipAppPro</h4>
            ...
            <div className="p-3 text-[11px] leading-relaxed space-y-2.5 border-b border-slate-200 dark:border-slate-800">
              <p className="text-slate-300">
                Bu uygulama, kamu kurumları ve özel işletmelerin salon, tesis ve mekan kiralama süreçlerini standartlaştırmak, resmi tarife ve tahsis evraklarını hatasız yönetmek amacıyla geliştirilmiş <strong>yerel SQLite tabanlı kurumsal masaüstü sistemdir.</strong>
              </p>
              <div className="bg-indigo-500/10 border border-indigo-500/20 p-2.5 rounded-xl">
                ...
                <p className="text-[10px] text-slate-300 leading-relaxed font-medium">
                  Merkezi ağ üzerinde ortak veri havuzu, bulut e-fatura / makbuz entegrasyonu, rol bazlı yetkilendirme (RBAC) ve Google Drive yedekleme çözümleri için geliştiriciyle iletişime geçebilirsiniz.
                </p>
              </div>
            </div>
            ...
            <div className="p-1.5 flex flex-col gap-0.5 bg-slate-50/50 dark:bg-slate-950/40">
              <button
                type="button"
                onClick={() => openExternal("https://github.com/ilyasbozdemir/kira-takip-electron")}
              ...
              <button
                type="button"
                onClick={() => openExternal("https://github.com/ilyasbozdemir/kira-takip-electron/issues")}
                className="flex items-center gap-2 w-full p-2 text-xs hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
              >
